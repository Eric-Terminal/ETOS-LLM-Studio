...
*   **会话管理升级（3.27 ~ 4.06）**：支持 iOS / watchOS 会话全文检索、消息序号定位、会话文件夹分类、批量移动与单会话跨端发送。
*   **导入导出能力补齐**：新增第三方导入（Cherry Studio、RikkaHub、Kelivo、ChatGPT conversations），并支持会话导出 PDF / Markdown / TXT。
*   **同步与备份 2.0**：新增 ETOS 数据包导入导出、手表端全量导入，以及「输入地址后直接 POST 上传导出包」能力。
*   **显示系统大改版**：支持自定义字体（含 WOFF / WOFF2）、字体样式槽位优先级、气泡/文字颜色配置、无气泡 UI 与思考自动预览默认开启。
*   **Markdown 与代码块体验增强**：新增代码高亮、复制反馈、折叠切换、iOS 可预览代码块、Mermaid 图表渲染与引用块竖线样式。
*   **工具中心继续扩展**：新增 Agent Skills 全链路接入与导入能力、ask_user_input 结构化问答工具、显示网页卡片工具与反馈工单自动提交工具。
*   **网络与语音能力增强**：支持全局/提供商级 HTTP(S)/SOCKS 代理（含鉴权），并接入系统流式 STT 语音输入（iOS + watchOS）。
*   **反馈与通知体验升级**：反馈中心支持工单内评论对话、开发者标记展示、状态自动刷新，以及对应的高优先级本地通知跳转。
...
从最初那个只有 1,800 行代码、API Key 还要硬编码的简陋版本，到现在 **223 个 Swift 源文件、119,112 行代码**（含 Shared / iOS / watchOS / 测试代码）的工程，它确实已经长大了不少。虽然名字叫 “ETOS LLM Studio” 听着有点唬人，但它本质上还是我拿来探索大模型应用边界的试验场。
...
*   **跨端同步**：内置 iOS ↔ watchOS 同步引擎，提供商配置、会话、世界书、工具配置、每日脉冲等数据可自动互通。
*   **同步与备份**：支持 ETOS 数据包导出/导入、手表端全量导入，以及通过自定义地址直接上传导出包。
...
*   **数据**: SQLite（本地向量数据库）, JSON（配置与数据持久化）
...
Shared/SharedTests/                         ← Shared 层测试（49 个 Swift 源文件）
```
...
本次 README 修订于 2026 年 4 月 8 日（99b6f19 之后）。项目更新频率比较高，如果你发现 README 跟不上代码，欢迎直接翻提交记录。
