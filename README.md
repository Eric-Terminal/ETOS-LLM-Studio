...
现在它已经不只是一个手表端 App 了：iOS、iPadOS 和 watchOS 都能管理云端模型、本地 GGUF 权重、工具、记忆、世界书和每日脉冲；Agent 还可以操作受控浏览器、调用如快捷指令的 Apple 原生能力，或在 App 内置的本地 Linux 中运行命令与 Skill 脚本。既可以通过 iOS 和 watchOS 的同步引擎进行同步互通，对于多台设备，也可以通过 iCloud 将配置、对话等在多个设备间流转。

因为我平时主要还是用 Mac 和 Watch，iOS 端偶尔会有一些未被发现的 bug，不过我会持续维护。
