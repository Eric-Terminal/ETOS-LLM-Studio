...
*   **Apple-Native Tools**: Four catalogs define 64 device, media/environment, personal-data, and vision/language capabilities. Only tools supported by the current platform, OS version, and hardware are shown. watchOS omits unsupported native tools and does not delegate them to iPhone; supported weather, HomeKit, and Bluetooth operations run locally. Side effects require per-call approval. See the [support and developer configuration checklist](../native-tools-support.md).
...
├── NativeCapabilities/                 ← Apple-native tool definitions, executors, permissions, and local availability
