...
*   **Apple ネイティブツール**：デバイス、メディアと環境、個人データ、ビジョンと言語の 4 グループに計 64 項目を定義し、端末、OS バージョン、ハードウェアが対応するツールのみ表示します。watchOS は非対応のネイティブツールを表示せず、iPhone への代行も行いません。天気、HomeKit、Bluetooth などの対応機能は端末上で実行します。書き込みや外部への操作には毎回承認が必要です。
...
├── NativeCapabilities/                 ← Apple ネイティブ機能の定義、実行、権限、端末上の対応状況
