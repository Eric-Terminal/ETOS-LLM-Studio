...
このプロジェクトは、プラットフォーム非依存の ETOSCore フレームワークと、各プラットフォーム専用のビュー層からなる二層構造です。最新のリファクタで `Config/AppConfigStore` 設定ハブを導入して `@AppStorage` を全面的に置き換え、さらに `LocalLLM` / `LocalLLMBridge` により端末上の GGUF 推論を既存のチャットライフサイクルへ接続しました。現在最大の Swift ファイルは約 1,365 行（`Config/AppConfigStore.swift`）で、ローカルモデル管理画面と生成パラメータマッピングは今後も少しずつ軽くしていく重めのモジュールです。

```
ETOSCore/ETOSCore/                         ← プラットフォーム非依存の業務ロジック（278 個の Swift ソースファイル）
...
ETOSCore/ETOSCoreTests/                         ← ETOSCore 層テスト（96 個の Swift ソースファイル）
```
