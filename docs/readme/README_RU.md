...
*   **Нативные инструменты Apple**: четыре каталога определяют 64 возможности устройства, медиа и окружения, личных данных, зрения и языка. Отображаются только инструменты, поддерживаемые текущей платформой, версией ОС и оборудованием. watchOS скрывает неподдерживаемые нативные инструменты и не передаёт их выполнение на iPhone; поддерживаемые операции погоды, HomeKit и Bluetooth выполняются локально. Запись и внешние действия требуют подтверждения при каждом вызове.
...
├── NativeCapabilities/                 ← Определения, исполнители, права и локальная доступность нативных Apple-инструментов
