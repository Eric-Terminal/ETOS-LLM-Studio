...
**Нативный AI‑клиент для iOS и Apple Watch. Поддерживает OpenAI, Anthropic Claude, Google Gemini и совместимые API, вызов MCP‑инструментов, локальную RAG‑память, Worldbook, Daily Pulse, Siri Shortcuts и синхронизацию между устройствами.**
...
*   **Обновлённое управление чатами (27.03 ~ 06.04)**: полнотекстовый поиск на iOS/watchOS, переход к сообщению по номеру, папки, массовое перемещение и отправка отдельного чата на другое устройство.
*   **Расширенный импорт/экспорт**: добавлен импорт из Cherry Studio, RikkaHub, Kelivo, ChatGPT conversations; экспорт чатов в PDF / Markdown / TXT.
*   **Синхронизация и резервирование 2.0**: ETOS‑пакеты для импорта/экспорта, полный импорт на watchOS и загрузка экспортного пакета через POST на указанный адрес.
*   **Большое обновление отображения**: пользовательские шрифты (включая WOFF / WOFF2), приоритеты слотов шрифтов, цвета пузырей/текста, bubbleless UI и автопредпросмотр «мыслей» по умолчанию.
*   **Улучшен Markdown и кодовые блоки**: подсветка синтаксиса, обратная связь при копировании, сворачивание кода, предпросмотр кода на iOS, Mermaid‑диаграммы и стиль цитат с вертикальной линией.
*   **Расширенный Tool Center**: полный поток Agent Skills с импортом, структурированный `ask_user_input`, инструмент отображения web‑виджетов и автоотправка тикета в Feedback Assistant.
*   **Сеть и голос**: глобальный и per‑provider HTTP(S)/SOCKS прокси (с авторизацией), плюс потоковый STT на iOS и watchOS.
*   **Feedback и уведомления**: комментарии внутри тикета, отметка ответов разработчика, автообновление статуса и переход из high‑priority уведомления в нужный тикет.
...
Изначально это был маленький эксперимент: около 1 800 строк и даже захардкоженные API‑ключи. Сейчас проект вырос до **223 Swift‑файлов и 119 112 строк кода** (Shared / iOS / watchOS / tests). Название «ETOS LLM Studio» звучит громко, но по сути это всё ещё мой полигон для экспериментов с LLM‑приложениями.
...
*   **Синхронизация iOS ↔ watchOS**: провайдеры, сессии, worldbook, настройки инструментов, Daily Pulse и другое.
*   **Синхронизация и резервирование**: экспорт/импорт ETOS‑пакетов, полный импорт на watchOS, загрузка пакетов на пользовательский endpoint.
...
*   **Данные**: SQLite (локальная векторная БД), JSON (настройки и хранение данных)
...
*   **Зависимости**: Swift Package Manager (`swift-markdown-ui` + transitives `networkimage`, `swift-cmark`)
...
```
Shared/Shared/                  ← Общая бизнес-логика (83 Swift-файла)
...
├── Persistence.swift            ← Конфигурация и сохранение данных
...
ETOS LLM Studio/ETOS LLM Studio Watch App/  ← UI-слой watchOS (47 Swift-файла)
Shared/SharedTests/                         ← Тесты Shared-слоя (49 Swift-файлов)
```
...
Этот README обновлён 8 апреля 2026 года (после 99b6f19). Если README не успел за кодом, смотрите историю коммитов.
