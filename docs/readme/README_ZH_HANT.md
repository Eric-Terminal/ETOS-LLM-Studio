...
**一個運行於 iOS 和 Apple Watch 的原生 AI 客戶端。支援 OpenAI、Anthropic Claude、Google Gemini 與多種相容提供商，內建 MCP 工具調用、本地 RAG 記憶、世界書、每日脈衝、Siri 捷徑與雙端同步。**
...
從最初那個只有 1,800 行程式碼、API Key 還要硬編碼的粗糙版本，到現在擁有 **235 個 Swift 原始碼檔案、118,070 行程式碼**（含 Shared / iOS / watchOS / 測試）的工程，它確實已經長大了不少。雖然「ETOS LLM Studio」這個名字聽起來有點唬人，但本質上它還是我拿來探索大模型應用邊界的試驗場。
...
*   **會話管理增強**：支援會話全文檢索、訊息序號定位、資料夾分類、批次移動，以及單會話跨端發送。
...
*   **會話匯入匯出**：支援匯入 Cherry Studio、RikkaHub、Kelivo、ChatGPT conversations 等第三方會話，並可匯出 PDF / Markdown / TXT。
*   **語音輸入（STT）**：接入系統 `SFSpeechRecognizer` 流式辨識，錄音面板可即時轉寫並一鍵回填輸入框。
*   **語音朗讀（TTS）**：支援系統 TTS、雲端 TTS 與自動回退，可獨立選擇 TTS 模型與朗讀參數。

#### 顯示與閱讀體驗

*   **顯示系統可自訂**：支援自訂字體（含 WOFF / WOFF2）、字體樣式槽位優先級、氣泡/文字顏色配置與無氣泡 UI。
*   **字體回退策略**：支援整段/單字粒度的字體回退範圍設定，提升中英混排與符號場景的穩定性。
*   **思考與內容預覽**：思考自動預覽預設開啟，減少手動展開操作。
*   **Markdown 與程式碼區塊增強**：支援語法高亮、複製回饋、折疊切換、iOS 程式碼預覽、Mermaid 渲染與引用區塊左側豎線樣式。
...
*   **拓展工具能力補齊**：新增 SQLite 資料庫增刪改查、網頁卡片顯示與反饋工單自動提交工具。
...
*   **GRDB 關聯式持久化**：核心資料持久化從 JSON 遷移到 GRDB + SQLite，覆蓋會話、配置、MCP、世界書、記憶、反饋、捷徑等模組。
...
*   **跨端同步**：內建 iOS ↔ watchOS 同步引擎，可自動同步提供商配置、會話、世界書、工具設定、每日脈衝資料等內容，並支援 Manifest/Delta 差異同步主鏈路。
*   **同步與備份**：支援 ETOS 數據包導出/匯入、手錶端全量匯入、啟動備份與損壞自癒，以及透過 S3 相容物件儲存（AWS S3 / Cloudflare R2）簽名上傳快照。
...
*   **資料**: GRDB + SQLite（會話 / 配置 / 記憶等核心持久化與本地向量資料庫）, JSON（匯入匯出與相容格式）
...
*   **依賴管理**: Swift Package Manager（當前顯式依賴 `GRDB.swift`、`swift-markdown-ui`，並包含其傳遞依賴 `networkimage` 與 `swift-cmark`）
...
```
Shared/Shared/                  ← 平台無關的業務邏輯（87 個 Swift 原始碼檔案）
├── ChatService.swift            ← 管理會話、訊息、模型選擇與請求編排的核心單例
├── APIAdapter.swift             ← OpenAI / Anthropic / Gemini 等 API 適配層
├── Models.swift                 ← 核心資料模型
├── Persistence.swift            ← 存儲入口、遷移觸發與生命週期協調
├── PersistenceGRDBStore.swift   ← GRDB 關聯式持久化核心實作
...
Shared/SharedTests/                         ← Shared 層測試（54 個 Swift 原始碼檔案）
```
...
本次 README 修訂於 2026 年 4 月 18 日（31d1e21 之後）。專案更新速度很快，如果 README 一時跟不上程式碼，最準的還是提交記錄。
