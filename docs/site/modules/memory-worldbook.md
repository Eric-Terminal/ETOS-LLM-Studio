...
角色扮演的当前会话设置提供三个彼此独立的开关：

- **屏蔽记忆**：不读取长期记忆、跨会话摘要和用户画像，也不提供记忆写入与主动检索工具
- **屏蔽工具**：不向模型提供 App、MCP、Agent Skills 与快捷指令工具，并移除历史工具调用消息；记忆上下文仍可保留
- **屏蔽全局系统提示词**：只移除全局系统提示词，会话提示词、角色卡、用户身份和世界书继续发送

这些开关不依赖是否绑定世界书。它们适合**角色扮演和绘画提示词场景**：既能避免全局设定串入，又能按需保留工具或记忆。
