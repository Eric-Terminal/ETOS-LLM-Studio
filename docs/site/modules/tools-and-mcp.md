...
原生工具只显示当前设备、系统版本和硬件支持的能力。Apple Watch 不提供剪贴板、AlarmKit 闹钟、NFC、音频文件转写、系统照片图库、Vision 图像分析，以及联系人、日历和提醒事项写入；这些能力不会委托给 iPhone。手表支持的天气、HomeKit、蓝牙、联系人读取等工具在手表本机执行。

工具出现在列表里以后，仍可能需要系统授权、网络或兼容配件。通知和媒体工具只管理 ETOS 自己创建的通知与播放器。路线计算支持驾车、步行和骑行；公交路线可以使用地图打开工具查看。手表的系统 URL 工具仅接受 `tel` 和 `sms`。
...
机制：当一个会话开启**屏蔽工具**时，工具列表会显示「当前会话已屏蔽相关上下文，因此不会实际启用该工具」，AI 在这个会话里调不到任何工具。此开关独立于记忆与全局系统提示词。

详见 [记忆与世界书](/modules/memory-worldbook) 里的“会话上下文屏蔽”。
