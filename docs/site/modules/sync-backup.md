...
快照会保留数据库中的 Linux 配置，但不包含本地 Linux 的系统、Home、Shared、工作区或导出文件。卸载后恢复快照，首次使用 Linux 时会重新安装内置系统，之前安装的软件和工作区文件不会随之恢复；外部文件夹可能需要重新授权。重置 Linux 会停止运行并清除系统，下次使用时再安装，重置本身不依赖启动成功。
...
选择 `.elsbackup` 时，会进入与「从快照恢复」相同的恢复确认流程，支持普通和加密快照，并替换快照涵盖的数据库。watchOS 从链接下载 `.elsbackup` 后也采用此流程。旧版 ETOS JSON 数据包和其他客户端数据继续按导入预览执行合并。
