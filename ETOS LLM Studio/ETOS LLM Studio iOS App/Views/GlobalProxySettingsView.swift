        ...
        Form {
            Section(
                ...
                if proxyStore.isEnabled {
                    ...
                    TextField("代理地址", text: $proxyStore.host)
                        .keyboardType(.URL)
                    ...
                    TextField("用户名（可选）", text: $proxyStore.username)
                        .textInputAutocapitalization(.never)
                        .autocorrectionDisabled()

                    Group {
                        if showPassword {
                            TextField("密码（可选）", text: $proxyStore.password)
                        } else {
                            SecureField("密码（可选）", text: $proxyStore.password)
                        }
                    }
                    .textInputAutocapitalization(.never)
                    .autocorrectionDisabled()
