...
struct WorldbookSettingsView: View {
    ...
    private var guideSettings: [GuidePageSetting] {
        var settings: [GuidePageSetting] = [
            .bool(
                "model_picker_shortcut_enabled",
                label: NSLocalizedString("在模型选择器中显示世界书", comment: "世界书向导字段"),
                get: { appConfig.modelPickerWorldbookShortcutEnabled },
                set: { appConfig.modelPickerWorldbookShortcutEnabled = $0 }
            ),
            .readOnly("worldbooks", label: NSLocalizedString("已导入世界书", comment: "世界书向导字段"), value: {
                .array(worldbooks.map { book in
                    .dictionary([
                        ...
                        "bound_to_current_session": .bool(isBoundToCurrentSession(book))
                    ])
                })
            })
        ]
    ...
    private var boundWorldbooksSetting: GuidePageSetting {
        .json(
            ...
            schema: .dictionary([
                ...
                "items": .dictionary([
                    "type": .string("string"),
                    "enum": .array(worldbooks.map { .string($0.id.uuidString) })
                ]),
            ...
            set: { value in
                guard var session = viewModel.currentSession,
                      case .array(let values) = value else { throw GuideError.invalidToolArguments }
