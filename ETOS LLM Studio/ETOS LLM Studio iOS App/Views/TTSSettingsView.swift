...
struct TTSSettingsView: View {
    ...
    private var guideSettings: [GuidePageSetting] {
        [
            ...
            .bool("filter_code_and_html", label: NSLocalizedString("过滤代码与 HTML 标签", value: "Filter Code and HTML Tags", comment: "TTS 内容过滤开关"), get: { appConfig.ttsFilterCodeAndHTML }, set: { appConfig.ttsFilterCodeAndHTML = $0 }),
    ...
    private var behaviorSection: some View {
        Section {
            ...
            Toggle(
                NSLocalizedString("过滤代码与 HTML 标签", value: "Filter Code and HTML Tags", comment: "TTS 内容过滤开关"),
                isOn: $appConfig.ttsFilterCodeAndHTML
            )
        } header: {
            Text(NSLocalizedString("朗读行为", comment: ""))
        } footer: {
            VStack(alignment: .leading) {
                Text(NSLocalizedString("所选内容为空时会朗读正文；开启过滤后，不会恢复已过滤的代码。", value: "If the selection is empty, the body text is read. Filtered code is never restored.", comment: "TTS 筛选回退说明"))
    ...
    private var settingsIntroCard: some View {
        SettingsHelpCard(
            title: NSLocalizedString("朗读内容过滤", value: "Spoken Content Filtering", comment: "TTS 介绍卡标题"),
            summary: NSLocalizedString("保留 HTML 正文，跳过代码、脚本和样式。", value: "Keep HTML body text and skip code, scripts, and styles.", comment: "TTS 过滤摘要")
        ) {
            SettingsHelpText(NSLocalizedString("TTS 内容过滤使用说明", value: "Enable Filter Code and HTML Tags to keep body text from HTML blocks and raw HTML, while skipping other fenced code blocks and inline code. Filtering runs before quote, parentheses, and italic selection. An empty selection falls back only to the filtered body; if no body remains, playback is skipped. This also applies to lightweight preprocessing on Apple Watch.\n\nOnly text present in the HTML source is extracted. Scripts are not run, so text generated by scripts or loaded externally cannot be extracted.", comment: "TTS 过滤详细教程"))
                .guideSettingsPageContext(
                    id: "tts-filtering-introduction",
                    title: NSLocalizedString("朗读内容过滤", value: "Spoken Content Filtering", comment: "TTS 介绍页标题"),
                    documents: [GuideDocumentReference(id: "tts", title: "Text to Speech")],
                    settings: [.readOnly("read_only", label: NSLocalizedString("朗读内容过滤", value: "Spoken Content Filtering", comment: "TTS 介绍页只读状态"), value: { .bool(true) })]
                )
        }
    }
