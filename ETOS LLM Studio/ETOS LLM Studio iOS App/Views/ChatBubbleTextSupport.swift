...
struct StreamingMessageSweepModifier: ViewModifier {
    ...
    var bandWidthRatio: CGFloat = 0.38
            ...
            .overlay {
                if isActive && !reduceMotion {
                    GeometryReader { proxy in
                        let width = proxy.size.width
                        let height = proxy.size.height
                        let bandWidth = max(36, width * bandWidthRatio)
                        let bandHeight = max(24, height * 1.35)
                        let startX = -bandWidth
                        let endX = width + bandWidth
                            ...
                            Rectangle()
                                .fill(
                                    LinearGradient(
                                        stops: [
                                            .init(color: .clear, location: 0),
                                            .init(color: highlightColor.opacity(opacity), location: 0.45),
                                            .init(color: highlightColor.opacity(opacity * 0.65), location: 0.58),
                                            .init(color: .clear, location: 1)
                                        ],
                                        startPoint: .leading,
                                        endPoint: .trailing
...
extension View {
    func streamingMessageSweep(
        ...
        duration: Double = 1.45,
        bandWidthRatio: CGFloat = 0.38,
        opacity: Double = 0.18
    ) -> some View {
        modifier(
