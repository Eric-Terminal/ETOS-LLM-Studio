...
struct GlobalProxySettingsView: View {
    ...
    var body: some View {
        ...
        .guidePageContext(
            descriptor: GuidePageDescriptor(
                id: "watch-global-proxy",
                title: NSLocalizedString("全局代理设置", comment: "手表全局代理向导上下文标题"),
    ...
    private func proxyGuideSnapshot() async -> GuidePageSnapshot {
        GuidePageSnapshot(fields: [
            "enabled": GuideSnapshotField(label: NSLocalizedString("启用全局代理", comment: "手表全局代理向导快照字段"), value: .bool(proxyStore.isEnabled)),
            "type": GuideSnapshotField(label: NSLocalizedString("代理类型", comment: "手表全局代理向导快照字段"), value: .string(proxyStore.type.rawValue)),
            "host": GuideSnapshotField(label: NSLocalizedString("代理地址", comment: "手表全局代理向导快照字段"), value: .string(proxyStore.host)),
            "port": GuideSnapshotField(label: NSLocalizedString("端口", comment: "手表全局代理向导快照字段"), value: .int(proxyStore.port)),
            "username": GuideSnapshotField(label: NSLocalizedString("用户名", comment: "手表全局代理向导快照字段"), value: .string(proxyStore.username)),
            "password": GuideSnapshotField(label: NSLocalizedString("密码", comment: "手表全局代理向导快照字段"), value: .string(proxyStore.password), access: .writeOnly)
        ])
    }
    ...
    private func buildProxyGuideProposal(
        ...
        let labels: [String: String] = [
            "enabled": NSLocalizedString("启用全局代理", comment: "手表全局代理向导修改字段"),
            "type": NSLocalizedString("代理类型", comment: "手表全局代理向导修改字段"),
            "host": NSLocalizedString("代理地址", comment: "手表全局代理向导修改字段"),
            "port": NSLocalizedString("端口", comment: "手表全局代理向导修改字段"),
            "username": NSLocalizedString("用户名", comment: "手表全局代理向导修改字段"),
            "password": NSLocalizedString("密码", comment: "手表全局代理向导修改字段")
        ]
        ...
        return GuideActionProposal(
            pageID: "watch-global-proxy",
            toolCallID: call.id,
            toolName: call.toolName,
            summary: NSLocalizedString("修改全局代理配置", comment: "手表全局代理向导提案摘要"),
            mutations: mutations,
            arguments: arguments
        )
    }
    ...
    private func executeProxyGuideProposal(_ proposal: GuideActionProposal) async throws -> GuideActionExecution {
        ...
        guard !updated.isEnabled || updated.normalizedIfEnabled != nil else {
            throw GuideError.invalidToolArguments
        }
        ...
        return GuideActionExecution(
            message: NSLocalizedString("已保存全局代理配置。", comment: "手表全局代理向导执行结果"),
