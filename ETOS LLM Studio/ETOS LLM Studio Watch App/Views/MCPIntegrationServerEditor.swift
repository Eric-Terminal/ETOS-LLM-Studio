...
struct MCPServerEditor: View {
    ...
    private var guidePageID: GuidePageID {
        GuidePageID(rawValue: "watch-mcp-server-editor-\(existingServer?.id.uuidString.lowercased() ?? "new")")
    }

    private var guidePageTitle: String {
        existingServer == nil
            ? NSLocalizedString("新增服务器", comment: "MCP 编辑器向导标题")
            : NSLocalizedString("编辑服务器", comment: "MCP 编辑器向导标题")
    }
    ...
    private var guideSettings: [GuidePageSetting] {
        var settings: [GuidePageSetting] = [
            ...
            .string("display_name", label: NSLocalizedString("显示名称", comment: "MCP 编辑器向导字段"), allowsEmpty: false, get: { displayName }, set: { displayName = $0 }),
            .string("notes", label: NSLocalizedString("备注", comment: "MCP 编辑器向导字段"), get: { notes }, set: { notes = $0 })
        ]

        if transportOption.isBuiltIn {
            settings.append(.readOnly("transport_type", label: NSLocalizedString("传输类型", comment: "MCP 编辑器向导字段"), value: { .string(transportOption.rawValue) }))
            return settings
        }
        ...
        settings.append(.string(
            ...
            set: { rawValue in
                if let option = TransportOption(rawValue: rawValue) { transportOption = option }
            }
        ))

        // 二级向导固定来源页声明；完整字段集允许一次完成协议切换与对应草稿填写。
        settings.append(contentsOf: [
            .string("endpoint", label: NSLocalizedString("Streamable HTTP Endpoint", comment: "MCP 编辑器向导字段"), allowsEmpty: false, get: { endpoint }, set: { endpoint = $0 }),
            .string("sse_endpoint", label: NSLocalizedString("SSE Endpoint", comment: "MCP 编辑器向导字段"), allowsEmpty: false, get: { sseEndpoint }, set: { sseEndpoint = $0 })
        ])
    ...
    private func appendHTTPSecretSettings(to settings: inout [GuidePageSetting]) {
        settings.append(.writeOnlyString("api_key", label: NSLocalizedString("Bearer API Key（只能写入，不能读取）", comment: "MCP 编辑器向导字段"), isConfigured: { !apiKey.isEmpty }, set: { apiKey = $0 }))
    ...
    private var oauthGuideSettings: [GuidePageSetting] {
        [
            ...
            .string("grant_type", label: NSLocalizedString("授权类型", comment: "MCP 编辑器向导字段"), allowedValues: MCPOAuthGrantType.allCases.map(\.rawValue), allowsEmpty: false, get: { oauthGrantType.rawValue }, set: { rawValue in
                if let grantType = MCPOAuthGrantType(rawValue: rawValue) { oauthGrantType = grantType }
            }),
    ...
    private var localStdioGuideSettings: [GuidePageSetting] {
        [
            .writeOnlyString("local_configuration_json", label: NSLocalizedString("stdio 配置 JSON（可能含环境变量，只能整体写入）", comment: "MCP 编辑器向导字段"), isConfigured: { !localConfigurationJSON.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty }, set: { localConfigurationJSON = $0 }),
            ...
            .string("launch_policy", label: NSLocalizedString("启动策略", comment: "MCP 编辑器向导字段"), allowedValues: MCPLocalStdioLaunchPolicy.allCases.map(\.rawValue), allowsEmpty: false, get: { localLaunchPolicy.rawValue }, set: { rawValue in
                if let policy = MCPLocalStdioLaunchPolicy(rawValue: rawValue) { localLaunchPolicy = policy }
            }),
            .string("idle_policy", label: NSLocalizedString("空闲策略", comment: "MCP 编辑器向导字段"), allowedValues: MCPLocalStdioIdlePolicy.allCases.map(\.rawValue), allowsEmpty: false, get: { localIdlePolicy.rawValue }, set: { rawValue in
