    ...
    var body: some View {
        List {
            Section {
                ...
                Text(NSLocalizedString("终端会按照这里的顺序横向显示；拖动可排序，轻扫可移除。", comment: "Watch 终端快捷键排序说明"))
                    .font(.footnote)
                    .foregroundStyle(.secondary)
            }

            if !availableShortcuts.isEmpty {
                Section(NSLocalizedString("添加按键", comment: "Watch 终端快捷键可添加分区")) {
                    ForEach(availableShortcuts) { shortcut in
                        Button {
                            addShortcut(shortcut)
                        } label: {
                            Label(shortcut.title, systemImage: "plus")
                        }
                    }
                }
            }
