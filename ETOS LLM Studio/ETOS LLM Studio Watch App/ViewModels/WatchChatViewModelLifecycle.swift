...
extension ChatViewModel {
    ...
    func setupSubscriptions() {
        ...
        // enableAdvancedRenderer 关闭时清空数学渲染覆盖集
        AppConfigStore.shared.$enableAdvancedRenderer
            .dropFirst()
            .receive(on: DispatchQueue.main)
        ...
        // backgroundOpacity 变更时归一化
        AppConfigStore.shared.$backgroundOpacity
            .dropFirst()
            .receive(on: DispatchQueue.main)
            .sink { [weak self] _ in self?.normalizeBackgroundOpacityIfNeeded() }
            .store(in: &cancellables)
