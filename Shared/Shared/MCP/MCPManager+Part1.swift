...
extension MCPManager {
    ...
    func autoConnectBackoffDelaySeconds(attempt: Int) -> TimeInterval {
        let exponent = max(0, attempt - 1)
        let delay = autoConnectBaseDelay * pow(2.0, Double(exponent))
        return min(delay, autoConnectMaxDelay)
    }

    func notifyAutoConnectFailureIfNeeded(for server: MCPServerConfiguration, error: Error) {
        let now = Date()
    ...
    func isInitializeTimeoutError(_ error: Error) -> Bool {
    ...
    func listResourcesIfSupported(client: MCPClient) async throws -> [MCPResourceDescription] {
        do {
    ...
    func listResourceTemplatesIfSupported(client: MCPClient) async throws -> [MCPResourceTemplate] {
        do {
    ...
    func listPromptsIfSupported(client: MCPClient) async throws -> [MCPPromptDescription] {
        do {
    ...
    func listRootsIfSupported(client: MCPClient) async throws -> [MCPRoot] {
        do {
