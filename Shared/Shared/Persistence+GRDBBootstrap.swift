...
extension Persistence {
    static func shouldUseGRDBStore() -> Bool {
    ...
    static func auxiliaryStoreKind(forKey key: String) -> AuxiliaryStoreKind {
    ...
    static func activeAuxiliaryStore(forKey key: String) -> PersistenceAuxiliaryGRDBStore? {
    ...
    static func legacyAuxiliaryStoreDatabaseURL(for kind: AuxiliaryStoreKind) -> URL {
        getChatsDirectory().appendingPathComponent(kind.rawValue, isDirectory: false)
    }

    static func migrateLegacyAuxiliaryStoreFileIfNeeded(kind: AuxiliaryStoreKind, targetURL: URL) {
    ...
    @discardableResult
    static func migrateLegacyAuxiliaryBlobIfNeeded(
        forKey key: String,
    ...
    @discardableResult
    static func cacheLaunchPreparationResult(_ result: LaunchPreparationResult) -> LaunchPreparationResult {
        launchBackupAndRecoveryLock.lock()
    ...
    enum LaunchBackupRestoreResult {
        case restored
        case missingBackup
        case failed
    }

    static func makeLaunchRecoveryNotice(from result: LaunchPreparationResult) -> String? {
    ...
    static func databaseURL(for kind: LaunchDatabaseKind) -> URL {
        switch kind {
        case .chat:
    ...
    static func launchBackupURL(for kind: LaunchDatabaseKind) -> URL {
    ...
    static func restoreDatabaseFromLaunchBackup(for kind: LaunchDatabaseKind) -> LaunchBackupRestoreResult {
        let databaseURL = databaseURL(for: kind)
    ...
    static func createLaunchBackup(for kind: LaunchDatabaseKind) throws {
        let sourceURL = databaseURL(for: kind)
        let fileManager = FileManager.default
    ...
    static func installVerifiedLaunchBackup(
        temporaryURL: URL,
        backupURL: URL,
        fileManager: FileManager
    ) throws {
    ...
    static func createChatLaunchBackupWithoutFTS(sourceURL: URL, destinationURL: URL) throws {
    ...
    static func copySQLiteDatabase(sourceURL: URL, destinationURL: URL) throws {
