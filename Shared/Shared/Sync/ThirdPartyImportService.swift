...
private extension ThirdPartyImportService {
    ...
    static func findJSONInDirectory(
        ...
        while let fileURL = enumerator?.nextObject() as? URL {
            if isDirectory(fileURL) { continue }
            guard nameSet.contains(fileURL.lastPathComponent.lowercased()) else { continue }
            if let parsed = tryParseDictionaryJSON(from: fileURL) {
                return parsed
            }
        }
