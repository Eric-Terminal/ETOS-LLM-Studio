...
public enum GuideDocumentCatalog {
    public static let documents: [GuideDocument] = [
        ...
        GuideDocument(
            ...
            content: """
            页面向导只在设置与配置页面工作。iPhone 上开启“在设置中显示页面向导”后会出现可拖动的向导按钮；Apple Watch 从设置列表进入向导页面。页面向导在本机保留上一段对话，重新打开 App 后可继续交流；清空上下文也会删除本机记录。对话不会加入主聊天会话列表或设备同步。工具记录只保存名称和状态，不保存原始参数与结果；待确认修改、撤销记录和密钥草稿只在本次运行期间保留，重启后不会继续执行。
        ...
        GuideDocument(
            ...
            长用户消息只按字符数截断，不限制行数。预览字符数可填写 1–100000，iOS 默认 1000，watchOS 默认 300；设置通过数据库保存并沿用显示设置的同步机制。iOS 在“对话框视觉”标签页配置，watchOS 在显示设置列表直接配置。修改后会在后台刷新当前会话的用户消息预览；消息原文、版本、附件、模型输入、复制和导出仍保持完整，全文可从消息“更多”中的“查看完整内容”打开。向导通过 user_message_preview_character_limit 字段生成提案，经原生确认后直接保存；iOS 切换标签页后不能执行旧标签页的提案。
        ...
        GuideDocument(
            ...
            “过滤代码与 HTML 标签”默认关闭，可在 iOS 与 watchOS 的“朗读行为”中开启。开启后保留 HTML 围栏和裸 HTML 的正文，跳过其他语言的围栏代码、行内代码、HTML 标签属性、注释、脚本、样式及显式隐藏的节点。先过滤再按引号、括号或斜体筛选，避免把 HTML 属性引号误读为台词；筛选为空时只回退到过滤后的正文，正文也为空则不朗读。手表开启轻量预处理时仍会过滤，字符上限在过滤后应用。只提取源码中的静态文字，不运行 JavaScript，不获取外部资源，也不计算外部样式，无法提取脚本生成或外部加载的文字。设置页介绍卡可展开查看使用说明。
