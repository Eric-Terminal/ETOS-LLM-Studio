...
public enum GuideDocumentCatalog {
    public static let documents: [GuideDocument] = [
        ...
        GuideDocument(
            ...
            输出设置的“自动重试”最多重试次数默认 3，范围 0–10，0 表示关闭，次数不包含首次请求。网络超时、断线及 HTTP 408、429、500、502、503、504、529 会按 1、2、4、8、16、30 秒等待后重试；认证、参数错误、模型明确拒绝和用户主动取消不重试。等待时在原回复气泡内显示“正在思考·重试(当前/上限)·剩余秒数 秒后重试”，秒数每秒更新；请求发出后移除倒计时，保留重试次数，接收到新的正文、推理或工具参数后恢复正常显示。重试过程中不新增错误气泡；点击停止可取消倒计时、等待和后续请求。

            流式中断时以已接收正文预填充恢复，将中断版本保留，新增内容与原文合并到新版本。只有推理或尚未完成的工具参数时重新请求当前一步，不执行残缺参数，不重复执行已完成工具或附件上传；正常 EOS 不触发自动重试。达到上限后保留正文及错误，仍可手动预填充。API 是否能严格接续取决于模型支持；已收到图片或音频的请求不会用文本预填充恢复。向导可通过 maximum_request_retries 字段提出修改，经原生确认后保存到数据库。
