...
public enum GuideDocumentCatalog {
    public static let documents: [GuideDocument] = [
        ...
        GuideDocument(
            ...
            输出设置的“自动重试”最多重试次数默认 3，范围 0–10，0 表示关闭，次数不包含首次请求。网络超时、断线及 HTTP 408、429、500、502、503、504、529 会按 1、2、4、8、16、30 秒等待后重试；认证、参数错误、模型明确拒绝和用户主动取消不重试。等待时在原回复气泡内显示“正在思考·重试(2/3)·2s”，秒数每秒更新；请求发出后移除倒计时，保留重试次数，接收到新的正文、推理或工具参数后恢复正常显示。重试过程中不新增错误气泡；点击停止可取消倒计时、等待和后续请求。
