...
public enum GuideDocumentCatalog {
    public static let documents: [GuideDocument] = [
        GuideDocument(
            ...
            content: """
            页面向导只在设置与配置页面工作。iPhone 上开启“在设置中显示页面向导”后会出现可拖动的向导按钮；Apple Watch 从设置列表进入向导页面。页面帮助与首次模型配置分别在本机保留上一段对话，重新打开 App 后可继续交流；清空上下文也会删除本机记录。对话不会加入主聊天会话列表或设备同步。工具记录只保存名称和状态，不保存原始参数与结果；待确认修改、撤销记录和密钥草稿只在本次运行期间保留，重启后不会继续执行。
