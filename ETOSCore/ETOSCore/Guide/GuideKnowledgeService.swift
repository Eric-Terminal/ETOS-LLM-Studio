...
public enum GuideDocumentCatalog {
    public static let documents: [GuideDocument] = [
        ...
        GuideDocument(
            ...
            输出设置的“自动重试”最多重试次数默认 3，范围 0–10，0 表示关闭，次数不包含首次请求。网络超时、断线及 HTTP 408、429、500、502、503、504、529 会按 1、2、4、8、16、30 秒等待后重试；认证、参数错误、模型明确拒绝和用户主动取消不重试。重试期间显示“正在思考·重试(当前/上限)”，接收到新的正文、推理或工具参数后恢复正常显示；点击停止可取消等待和后续请求。
