...
final class LocalLinuxTerminalScreen: @unchecked Sendable {
    ...
    private func consume(_ byte: UInt8) {
        ...
        case .oscEscape:
            parserState = byte == 0x5C ? .ground : .osc
        case .ignoredString:
    ...
    private func put(_ character: Character) {
        ...
        buffer.lines[row][column] = Cell(text: String(character), isContinuation: false)
        if width == 2, column + 1 < columns {
            buffer.lines[row][column + 1] = Cell(text: "", isContinuation: true)
        }
    ...
    private func resized(
        ...
        var lines = source.lines.map { line -> [Cell] in
            ...
            if line.count < targetColumns {
                return line + Array(repeating: .blank, count: targetColumns - line.count)
            }
            return line
        }
    ...
    private func renderedLine(_ line: [Cell]) -> String {
        var fragments: [String] = []
        fragments.reserveCapacity(line.count)
        for cell in line where !cell.isContinuation {
            fragments.append(cell.text.isEmpty ? " " : cell.text)
        }
        var value = fragments.joined()
        while value.last == " " {
            value.removeLast()
        }
        return value
