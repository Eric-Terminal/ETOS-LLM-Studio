...
public struct PromptMacroHelpSection: View {
        ...
        Section {
            ...
            DisclosureGroup(NSLocalizedString("查看可用宏", value: "Available macros", comment: "展开提示词宏列表")) {
                Text(NSLocalizedString(
                    "提示词宏语法说明",
                    value: "Use {{name}} or {name}; names are case-insensitive and may have surrounding spaces. Unknown macros stay unchanged. Values come from the device sending the request. Unavailable readings return unknown. Saved prompts keep the original template.",
                    comment: "提示词宏语法和取值边界"
                ))
                .font(.footnote)
            ...
            Text(NSLocalizedString(
                "提示词宏缓存说明",
                value: "Put changing values such as time and battery level in the enhancement prompt to preserve more of the request prefix for caching. Using them in system or topic prompts can reduce cache reuse. Cache support and reuse depend on the provider and API format.",
                comment: "动态宏的推荐位置与缓存边界"
            ))
            .font(.footnote)
            .foregroundStyle(.secondary)
        }
    ...
    private var settingsIntroCard: some View {
        VStack(alignment: .leading) {
            Text(NSLocalizedString(
                "提示词宏使用说明",
                value: "System prompts, conversation system prompts, topic prompts and enhancement prompts support macros. They are replaced with current values before each request. For example, put this in the enhancement prompt:",
                comment: "提示词宏使用教程"
            ))
            Text(NSLocalizedString(
                "提示词宏增强示例",
                value: "Current time: {{cur_datetime}} ({{timezone}})\nBattery (%): {{battery_level}}",
                comment: "可填入增强提示词的宏示例，保留宏原文"
            ))
            .monospaced()
        }
        .font(.footnote)
